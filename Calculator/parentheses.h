...
{
	cout << "( has been found!" << endl; //DEBUG
	...
	for (int search{ openParenthesis + 1 }; search < equation.size(); search++)
	{
		if (equation[search] == ")")
		{
			cout << "\ncurrently found:" << equation[search] << " at index: " << search; //DEBUG
	...
	cout << "\nclose par index:" << closeParenthesis << endl; //DEBUG
	...
	cout << "equation in () BEFORE operations:"; //DEBUG

	for (string check : equation_inPar) //DEBUG
		cout << check;

	cout << "\nexponant after par:" << par_Exponant; //DEBUG
	...
	cout << "\nDistributor number is:" << distributor; //DEBUG
